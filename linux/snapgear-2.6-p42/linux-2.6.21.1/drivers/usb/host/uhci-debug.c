...
#ifdef CONFIG_LEON_3
	usbcmd    = readw(&uhci->regs->usbcmd);
	usbstat   = readw(&uhci->regs->usbsts);
	usbint    = readw(&uhci->regs->usbintr);
	usbfrnum  = readw(&uhci->regs->frnum);
	flbaseadd = readw(&uhci->regs->flbaseadd);
	sof       = readw(&uhci->regs->sofmod);
	portsc1   = readw(&uhci->regs->portsc[0]);
	portsc2   = readw(&uhci->regs->portsc[1]);
#else
