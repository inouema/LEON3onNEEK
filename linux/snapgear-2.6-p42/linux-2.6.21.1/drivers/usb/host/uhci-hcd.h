...
#ifdef CONFIG_LEON_3
	struct uhci_ioregs __iomem *regs;
#else
	/* Grabbed from PCI */
...
#ifdef CONFIG_LEON
/* 
 * The GRUSBHC controller is connected via the AMBA APB bus.
 *  We access its registers the same way as the EHCI code.
 */
...
struct uhci_ioregs {
	...
	/* FRNUM: offset 0x0c */
	u16 frnum;
	/* FLBASEADD: offset 0x10 */
	u32 flbaseadd;
	/* SOFMOD: offset 0x14 */
	u32 sofmod;
	/* PORTSC1: offset: 0x18 */
	u16 portsc [0];
	/* FIXME: support for more ports */
