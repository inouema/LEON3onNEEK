...
#include <asm/leon.h>
#include <asm/irq.h>
#define MACADDR0 ((CONFIG_SMC91X_MAC_MSB >> 16) & 0xff)
#define MACADDR1 ((CONFIG_SMC91X_MAC_MSB >> 8) & 0xff)
#define MACADDR2 ((CONFIG_SMC91X_MAC_MSB >> 0) & 0xff)
#define MACADDR3 ((CONFIG_SMC91X_MAC_LSB >> 16) & 0xff)
#define MACADDR4 ((CONFIG_SMC91X_MAC_LSB >> 8) & 0xff)
#define MACADDR5 ((CONFIG_SMC91X_MAC_LSB >> 0) & 0xff)
...
#else	
	void *start = (void *)pdev;

	/*
	 * Map the attribute space.  This is overkill, but clean.
	 */
	addr = ioremap(start, ATTRIB_SIZE);
#endif
	if (!addr)
		return -ENOMEM;
...
static int __init smc_init(void)
{
	...
	unsigned int  irq_pio,irq_mctrl,addr_pio,addr_mctrl,addr_ftmctrl,addr_timer;
	unsigned char waitstate = 0xff;
   unsigned int tmp;
   int ret = 0;
	...
	if ((addr_pio = amba_find_apbslv_addr(VENDOR_GAISLER, GAISLER_PIOPORT, &irq_pio)) &&
		((addr_mctrl = amba_find_apbslv_addr(VENDOR_ESA, ESA_MCTRL, &irq_mctrl)) ||
		 (addr_ftmctrl = amba_find_apbslv_addr(VENDOR_GAISLER, GAISLER_FTMCTRL, &irq_mctrl))) &&
		(addr_timer = amba_find_apbslv_addr (VENDOR_GAISLER, GAISLER_GPTIMER, 0))) {
		if ( addr_ftmctrl ){
			addr_mctrl = addr_ftmctrl;
			printk("Activating Leon3 io port for smsc_lan91cxx (pio:%x ftmctrl:%x)\n",addr_pio,addr_mctrl);
		}else{
			printk("Activating Leon3 io port for smsc_lan91cxx (pio:%x mctrl:%x)\n",addr_pio,addr_mctrl);
		}
		addr_pio = (unsigned int)ioremap(addr_pio,24);
		addr_mctrl = (unsigned int)ioremap(addr_mctrl,4);
		addr_timer = (unsigned int)ioremap(addr_timer + 4, 4);
		waitstate = ((100*(*(volatile unsigned int*)addr_timer + 1))/1000);
		if ((waitstate & 0xf) != 0xf)
			waitstate++;
		LEON3_IOPORT_Regs_Map *io = (LEON3_IOPORT_Regs_Map *) addr_pio;
		tmp = *((volatile unsigned int *)addr_mctrl); //mctrl ctrl 1
		tmp &= 0x0107FFFF;
		tmp |= (0x10080000 | (waitstate & 0xf)<< 20);
		*((volatile unsigned int *)addr_mctrl) = tmp;
		io->irqmask |= (1 << CONFIG_SMC91X_PIO);
		io->irqpol  |= (1 << CONFIG_SMC91X_PIO);
		io->irqedge |= (1 << CONFIG_SMC91X_PIO);
		io->iodir &= ~(1 << CONFIG_SMC91X_PIO);
	} else {
  		return -ENXIO;
	}
	
	if (request_mem_region(CONFIG_SMC91X_BASE_ADDR, SMC_IO_EXTENT, "smc91x")){

		addr = ioremap(CONFIG_SMC91X_BASE_ADDR, SMC_IO_EXTENT); 
		if (!addr) {
			ret = -ENOMEM;
			release_mem_region(CONFIG_SMC91X_BASE_ADDR, SMC_IO_EXTENT);
			goto out_x;
		}
		ioaddr = (unsigned int)addr;
		
		smc_enable_device(CONFIG_SMC91X_BASE_ADDR);
		...
		if (ret == 0){
			unsigned int val;
			//Set MAC ADDR
			ndev->dev_addr[0] = MACADDR0;
			ndev->dev_addr[1] = MACADDR1;
			ndev->dev_addr[2] = MACADDR2;
			ndev->dev_addr[3] = MACADDR3;
			ndev->dev_addr[4] = MACADDR4;
			ndev->dev_addr[5] = MACADDR5;
			val = SMC_CURRENT_BANK();
			SMC_SELECT_BANK(1);
