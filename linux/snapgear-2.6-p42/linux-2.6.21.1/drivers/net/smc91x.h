...
/* SMC is not attached via PCI on LEON */
#define SMC_inb_(__addr)		__readb((void __iomem *)(unsigned long)(__addr))
#define SMC_inw_(__addr)		__readw((void __iomem *)(unsigned long)(__addr))
#define SMC_inl_(__addr)		__readl((void __iomem *)(unsigned long)(__addr))
#define SMC_outb_(__b, __addr)	__writeb(__b, (void __iomem *)(unsigned long)(__addr))
#define SMC_outw_(__w, __addr)	__writew(__w, (void __iomem *)(unsigned long)(__addr))
#define SMC_outl_(__l, __addr)	__writel(__l, (void __iomem *)(unsigned long)(__addr))

#define SMC_inb(a, r)		SMC_inb_((a) + (r))
#define SMC_inw(a, r)		SMC_inw_((a) + (r))
#define SMC_inl(a, r)		SMC_inl_((a) + (r))
#define SMC_outb(v, a, r)	SMC_outb_(v, (a) + (r))
#define SMC_outw(v, a, r)	SMC_outw_(v, (a) + (r))
#define SMC_outl(v, a, r)	SMC_outl_(v, (a) + (r))

#define SMC_raw_inl(a, r)	__raw_readl((a) + (r))
#define SMC_raw_outl(v, a, r)	__raw_writel(v, (a) + (r))
#define SMC_raw_outw(v, a, r)	__raw_writew(v, (a) + (r))

static inline void SMC_insl_(unsigned long a, int r, unsigned char* p, int l){
...
static inline void SMC_outsl_(unsigned long a, int r, unsigned char* p, int l){
   unsigned int *__p = (unsigned int*)p;
   while(l-- > 0){
       SMC_raw_outl(*(__p++), a, r);
   }
}

#define SMC_insl(a,r,p,l) SMC_insl_(a,r,p,l)
#define SMC_outsl(a,r,p,l) SMC_outsl_(a,r,p,l)
	...
	do {								\
		if (SMC_CAN_USE_32BIT) {				\
			...
			if (__len >= 2 && ((unsigned long)__ptr & 2)) {	\
				__len -= 2;				\
				SMC_raw_outw(*(u16 *)__ptr, ioaddr, DATA_REG); \
				__ptr += 2;				\
			}						\
			if (SMC_CAN_USE_DATACS && lp->datacs)		\
				__ioaddr = lp->datacs;			\
			SMC_outsl(__ioaddr, DATA_REG, __ptr, __len>>2);	\
			if ((unsigned long)__len & 2) {				\
				__ptr += ((unsigned long)__len & ~3);			\
				SMC_raw_outw(*((u16 *)__ptr), ioaddr, DATA_REG); \
			}						\
