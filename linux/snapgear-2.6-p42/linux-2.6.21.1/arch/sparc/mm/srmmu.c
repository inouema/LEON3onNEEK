 ...
 * Copyright (C) 2006 Konrad Eisele (eiselekd@web.de), Gaisler Research
 */
...
static int leon_flush_during_switch=-1;
#endif
...
#define SRMMU_NOCACHE_BITMAP_SHIFT (PAGE_MIN_SHIFT - 4)
...
void srmmu_nocache_calcsize(void)
{
	...
	srmmu_nocache_npages =
		sysmemavail / SRMMU_NOCACHE_ALCRATIO / 1024 * (256 /*>> CONFIG_PAGE_SIZE_LEON*/);
...
void __init srmmu_nocache_init(void)
{
	...
	printk("Nocache: 0x%x-0x%x, %d pages [%d-%d]\n",SRMMU_NOCACHE_VADDR, srmmu_nocache_end, srmmu_nocache_npages, SRMMU_MIN_NOCACHE_PAGES, SRMMU_MAX_NOCACHE_PAGES);
...
srmmu_pte_alloc_one_kernel(struct mm_struct *mm, unsigned long address)
{
	...
	return (pte_t *)srmmu_get_nocache(size,size/*PTE_SIZE, PTE_SIZE*/);
}
...
static void srmmu_free_pte_fast(pte_t *pte)
{
	...
	srmmu_free_nocache((unsigned long)pte, size /*PTE_SIZE*/);
}
...
static void srmmu_pte_free(struct page *pte)
{
	...
	srmmu_free_nocache(p, size /* PTE_SIZE*/);
}
...
struct thread_info *srmmu_alloc_thread_info(void)
{
	...
	/*printk("thread-info: 0x%x-0x%x\n",(int)ret, ((int)ret) + (PAGE_SIZE << THREAD_INFO_ORDER));*/
...
void __init leon_inherit_prom_mappings(unsigned long start,unsigned long end)
{
	...
	while(start <= end) {
		...
		ptep = srmmu_pte_offset(__nocache_fix(pmdp), start);
		*(pte_t *)__nocache_fix(ptep) = __pte((prompte & SRMMU_PTE_PMASK));
		start += PAGE_SIZE;
	}
}
...
static void leon_flush_tlb_page(struct vm_area_struct *vma, unsigned long page)
{
	struct mm_struct *mm = vma->vm_mm;
	unsigned long flags;
	
	FLUSH_BEGIN(mm)
	/* Stop interrupts from destroying */
	local_irq_save(flags);
	__asm__ __volatile__(
	"lda	[%0] %3, %%g5\n\t"		/* save current context in g5 */
	"sta	%1, [%0] %3\n\t"			/* Set MMU Context register to vma->vm_mm->context */
	"sta	%%g0, [%2] %4\n\t"		/* Flush MMU's Context Page 'page'  (vfpa=page,type=0,reserved=0)*/
	"sta	%%g5, [%0] %3\n"			/* Restore Context register from g5 */
	: /* no outputs */
	: "r" (SRMMU_CTX_REG), "r" (mm->context), "r" (page & PAGE_MASK),
	  "i" (ASI_LEON_MMUREGS), "i" (ASI_M_FLUSH_PROBE)
	: "g5");
...
void leon2_getCacheRegs(struct leon2_cacheregs *regs)
{
	unsigned long ccr,cfg;
	unsigned int  ccr_addr = 0x80000014, /* Hardcoded address of LEON2 Cache Configuration Register */
								cfg_addr = 0x80000024; /* Hardcoded address of LEON2 Configuration Register */
	
	if ( !regs )
		return;
	
	/* Get Cache regs from "Cache ASI" address 0x0, 0x8 and 0xC */
	__asm__ __volatile__( "lda [%2] %4, %0\n\t"
			      : "=r" (ccr), "=r" (cfg)  /* output */
			      : "r" (ccr_addr), "r" (cfg_addr), "i" (ASI_LEON_BYPASS) /* input */
			      );
	
	__asm__ __volatile__( "lda [%3] %4, %1\n\t"
			      : "=r" (ccr), "=r" (cfg)  /* output */
			      : "r" (ccr_addr), "r" (cfg_addr), "i" (ASI_LEON_BYPASS) /* input */
			      );
...
int leon_flush_needed(void)
{
	...
	char *setStr[4]={"direct mapped","2-way associative","3-way associative","4-way associative"};
	#ifdef CONFIG_LEON_3
		/* leon 3 */
	...
	#else
		/* leon 2 */
		struct leon2_cacheregs cregs;
		leon2_getCacheRegs(&cregs);
		sets = (cregs.ccr & LEON2_CCR_DSETS_MASK) >> 24;
		ssize = 1<<((cregs.cfg & LEON2_CFG_SSIZE_MASK) >> 12);
	#endif
...
static void __init init_leon(void) {
	...
	BTFIXUPSET_CALL(flush_cache_page, leon_flush_pcache_all, BTFIXUPCALL_NORM);
	BTFIXUPSET_CALL(flush_cache_range, leon_flush_cache_all, BTFIXUPCALL_NORM);
	BTFIXUPSET_CALL(flush_page_for_dma, leon_flush_dcache_all, BTFIXUPCALL_NORM);
