...
struct property * amba_property_create(char *n, int len, void *p) {
	struct property *head, *tail;
	head = (struct property *)prom_early_alloc(sizeof(struct property) + 32);
...
void amba_scan(struct device_node *dp, struct device_node ***nextp, amba_confarea_type* a_conf, unsigned int ioarea)
{
	...
	for (i = 0; i < a_conf->ahbslv.devnr; i++) {
		...
		mbar = amba_ahb_get_membar(a_conf->ahbslv, i, 0);
			...
			for (j = a_conf->apbslv.devnr, k = 0; j < AMBA_MAXAPB_DEVS && k < AMBA_MAXAPB_DEVS_PERBUS;
				...
				unsigned int apb_conf = LEON3_BYPASS_LOAD_PA(cfg_area);
				unsigned int iobar = LEON3_BYPASS_LOAD_PA(cfg_area+1);
				if (1 && (vendor = amba_vendor(apb_conf)) && (device = amba_device(apb_conf)))	{
					struct amba_prom_registers regs; unsigned int intr;
					struct device_node *node; struct property *head, *tail;
					node = prom_early_alloc(sizeof(*dp));
					node->unique_id = prom_unique_id++;
					node->parent = dp;
					node->node = node->unique_id ;
					node->name = device_id2str(amba_vendor(apb_conf),amba_device(apb_conf));
 					
					intr = amba_irq(apb_conf);
					regs.phys_addr = amba_iobar_start(apbmst, iobar);
					regs.reg_size = ((((iobar) & 0xfff0)<<4) | 0xff) + 1;
					
					node->properties = head = amba_property_create("reg", sizeof(regs), &regs);
					head = head ->next = amba_property_create("interrupts", sizeof(intr), &intr);
					head = head ->next = amba_property_create("vendor", sizeof(int), &vendor);
					head = head ->next = amba_property_create("device", sizeof(int), &device);
					...
					*next = node;
					...
					node->path_component_name = build_path_component(node);
					node->full_name = build_full_name(node);
					
					/*printk("Build: %s  [ %s ]\n", node->path_component_name, node->full_name);*/
...
void _amba_init(struct device_node *dp, struct device_node ***nextp)
{
	...
	_printk(("Reading AMBA Plug&Play configuration area\n"));
        ...
        amba_scan(dp, nextp, &amba_conf, LEON3_IO_AREA);
