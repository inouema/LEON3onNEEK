...
static int do_aoe(void)
{
	int fd = open("/dev/etherd/discover", O_WRONLY);

	if (fd < 0) {
		DEBUG(("Could not access ATA Over Ethernet device files\n"));
		return 0;
	}
...
int main(int argc, char *argv[])
{
	...
	/* Initialize ATA Over Ethernet devices, if available */
	do_aoe();
